...
在Python中定义函数，可以用必选参数、默认参数、可变参数、关键字参数和命名关键字参数，这5种参数都可以组合使用。但是请注意，参数定义的顺序必须是：<u>必选参数、默认参数、可变参数、命名关键字参数和关键字参数</u>。
...
但是有人说这样的话 如果我传入的参数是一个 list 或者tuple呢？这个好像不行诶？
...
<ipython-input-22-403d3f217d69> in <module>()
----> 1 product(*L)

TypeError: product() takes at least 1 argument (0 given)
```

后来参照网友的自己也背着写了一个参数更少的：
...
product(*L)
TypeError                                 Traceback (most recent call last)
<ipython-input-26-403d3f217d69> in <module>()
----> 1 product(*L)

<ipython-input-24-b3b50999fca2> in product(*num)
      1 def product(*num):
      2     if num is None or len(num) < 1 :
----> 3         raise TypeError("args not null！")
      4     else :
      5         sum = 1

TypeError: args not null！
```
